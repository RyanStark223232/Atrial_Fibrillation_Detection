...
def res_block(x, size):
    x_shortcut = x
    card = 32
    mid_size = size//card//2
    x_list = []
    for _ in range(card):
        x1 = k.layers.Conv1D(mid_size, 1, padding = 'same', kernel_regularizer=k.regularizers.l2(0.001))(x)
        x1 = k.layers.BatchNormalization()(x1)
        x1 = k.layers.LeakyReLU()(x1)
        x1 = k.layers.Dropout(rate = dropout)(x1)
        x1 = k.layers.Conv1D(mid_size, 9, padding = 'same', kernel_regularizer=k.regularizers.l2(0.001))(x)
        x1 = k.layers.BatchNormalization()(x1)
        x1 = k.layers.LeakyReLU()(x1)
        x1 = k.layers.Dropout(rate = dropout)(x1)
        x_list.append(x1)
    x = k.layers.Concatenate()(x_list)
    x = k.layers.Conv1D(size, 1, padding = 'same', kernel_regularizer=k.regularizers.l2(0.001))(x)
...
def down_scale(x, size, filt, strid):
    x = k.layers.Conv1D(size, filt, strides = strid, padding = 'same', kernel_regularizer=k.regularizers.l2(0.001))(x)
    x = k.layers.BatchNormalization()(x)
    x = k.layers.LeakyReLU()(x)
    x = k.layers.Dropout(rate = dropout)(x)
    return x
...
x = down_scale(x_input, 128, 49, 1)
x = res_block(x, 128)
x = k.layers.MaxPool1D()(x)
x = down_scale(x, 128, 9, 1)
#x = down_scale(x, 64, 9)
#x = res_block(x, 64, 9)
#x = down_scale(x, 16, 9)
#x = res_block(x, 16, 9)
...
for it in range(3000):
    for it in range(1):
        bat_index_set = np.random.choice(index_set[:512], size = 512, replace = False)
        bat_train_x = train_x[bat_index_set]
        bat_train_y = train_y[bat_index_set]
        model.fit(bat_train_x, bat_train_y, epochs = 1, batch_size = 16)
    preds = np.argmax(model.predict(test_x), axis = 1)
    result = np.concatenate([result, np.expand_dims(preds, axis = 0)], axis = 0)
    np.save("result/resnext_1", result)
model.save_weights("/research/ksleung5/hwong8/resnext_1.h5")